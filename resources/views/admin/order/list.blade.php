                    ...
                    <tbody>
                        @foreach ($orders as $items)
                            <tr>
                                ...
                                <td scope="col">
                                    @if ($items->status == 0)
                                        <a href="{{ route('order#details', $items->order_code) }}"
                                            class="text-dark btn-sm btn-warning rounded shadow-lg">{{ $items->order_code }}</a>
                                    @endif
                                    @if ($items->status == 1)
                                        <a href="{{ route('order#details', $items->order_code) }}"
                                            class="text-white btn-sm btn-success rounded shadow-lg">{{ $items->order_code }}</a>
                                    @endif
                                    @if ($items->status == 2)
                                        <a href="{{ route('order#details', $items->order_code) }}"
                                            class="text-white btn-sm btn-danger rounded shadow-lg">{{ $items->order_code }}</a>
                                    @endif
                                </td>
                                <td scope="col">{{ $items->user_name }}</td>
                                <td scope="col">
                                    <select name="" id="" class="form-select statusChange">
                                        <option value="0" @if ($items->status == 0) selected @endif>Pending
                                        </option>
                                        <option value="1" @if ($items->status == 1) selected @endif>Accept
                                        </option>
                                        <option value="2" @if ($items->status == 2) selected @endif>Reject
                                        </option>
                                    </select>
                                </td>
                                <td>
